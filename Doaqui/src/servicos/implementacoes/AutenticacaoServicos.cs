...
namespace Doaqui.src.servicos.implementacoes
{
    ...
    public class AutenticacaoServicos : IAutenticacao
    {
        ...
        /// <exception cref="Exception">Email não encontrado</exception>
        ...
        public async Task<AutorizacaoDTO> PegarAutorizacaoAsync(AutenticarDTO dto)
        {
            ...
            if (usuario == null) throw new Exception("Email não cadastrado");

            if (usuario.Senha != CodificarSenha(dto.Senha)) throw new Exception("Senha incorreta");

            return new  AutorizacaoDTO(usuario.Id, usuario.Email, usuario.Cnpj, usuario.Tipo, "Bearer " + GerarToken(usuario));
        }
